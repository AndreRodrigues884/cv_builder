  ...
  <div class="max-w-4xl mx-auto">
    ...
    <div v-if="!careerData" class="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center">
      ...
      <p class="text-slate-400 mb-6">
        A IA vai analisar o teu perfil e sugerir os melhores caminhos de carreira
      </p>
      ...
      <button @click="analyzeCareer" :disabled="aiStore.isLoading"
        ...
        <span v-if="aiStore.isLoading">A analisar...</span>
    ...
    <div v-else class="space-y-4">
      ...
      <div v-for="(path, idx) in careerData.suggestedPaths" :key="idx"
        class="bg-slate-900 border border-slate-800 rounded-xl p-6">
        <div class="flex items-start justify-between mb-4">
          <div>
            <h4 class="text-xl font-bold mb-1">{{ path.role }}</h4>
            <p class="text-sm text-slate-400">Timeline: {{ path.timeline }}</p>
          </div>
          <div class="text-right">
            <div :class="{
              'text-green-400': path.probability === 'high',
              'text-yellow-400': path.probability === 'medium',
              'text-red-400': path.probability === 'low'
            }" class="text-sm font-medium">
              {{ formatProbability(path.probability) }}
            </div>
            <div class="text-xs text-slate-400">{{ path.salary }}</div>
          </div>
        </div>
        ...
        <div class="bg-slate-800/50 rounded-lg p-4">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="(req, i) in path.requirements" :key="i"
      ...
      <div class="bg-gradient-to-br from-purple-600/10 to-pink-600/10 border border-purple-500/30 rounded-xl p-6">
        ...
        <ul class="space-y-2">
          <li v-for="(insight, idx) in careerData.insights" :key="idx"
...
export default {
  ...
  methods: {
    async analyzeCareer() {
      try {
        const result = await this.aiStore.suggestCareerPath()
        this.careerData = result
      } catch (error) {
